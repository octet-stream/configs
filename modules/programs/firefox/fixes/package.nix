{ ... }:
{
  flake.modules.homeManager.firefox =
    ...
    in
    {
      ...
      programs.firefox.package = lib.mkIf isDarwin (
        lib.mkDefault (lib.makeOverridable ({ ... }: pkgs.firefox-bin) { })
      );

      home.sessionVariables = lib.mkIf (isDarwin && config.programs.firefox.enable) {
